...
@section('content')
    ...
    <div class="pt-28 md:pt-32 max-w-lg mx-auto px-4 sm:px-6 pb-24">
        ...
        {{-- ============ RINGKASAN, DETAIL PEMESANAN & REKENING (SATU KARTU) ============ --}}
        <div class="bg-white border border-brand-100 rounded-2xl divide-y divide-brand-100 mb-6">

            {{-- Paket & Total --}}
            <div class="p-5 flex items-center justify-between gap-4">
                <div class="min-w-0">
                    <p class="font-serif font-semibold text-ink text-sm truncate">{{ $pemesanan->paket_nama }}</p>
                    <p class="text-xs text-ink/50 mt-0.5">
                        {{ \Carbon\Carbon::parse($pemesanan->tanggal_acara)->translatedFormat('d F Y') }} &middot; {{ $pemesanan->jam_acara }}
                    </p>
                </div>
                <div class="text-right shrink-0">
                    <p class="text-xs text-ink/50 mb-0.5">{{ __('messages.total_bayar_label') }}</p>
                    <p class="font-bold text-brand-600">Rp{{ number_format($pemesanan->paket_harga, 0, ',', '.') }}</p>
                </div>
            </div>
            ...
            <div class="p-5">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between gap-4">
                        <span class="text-ink/50 shrink-0">{{ __('messages.nama_label') }}</span>
                        <span class="font-medium text-ink text-right">{{ $pemesanan->nama_lengkap }}</span>
                    </div>
                    <div class="flex justify-between gap-4">
                        <span class="text-ink/50 shrink-0">{{ __('messages.no_telepon_label_pendek') }}</span>
                        <span class="font-medium text-ink text-right">{{ $pemesanan->telepon }}</span>
                    </div>
                    <div class="flex justify-between gap-4">
                        <span class="text-ink/50 shrink-0">{{ __('messages.lokasi_acara_label') }}</span>
                        <span class="font-medium text-ink text-right">{{ $pemesanan->alamat_acara }}</span>
                    </div>
                    @if ($pemesanan->catatan)
                        <div class="flex justify-between gap-4">
                            <span class="text-ink/50 shrink-0">{{ __('messages.catatan_label') }}</span>
                            <span class="font-medium text-ink text-right">{{ $pemesanan->catatan }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between gap-4">
        ...
        {{-- ============ TOMBOL AKSI ============ --}}
        <form method="POST" action="{{ route('rias.transfer.confirm') }}" enctype="multipart/form-data" class="space-y-3">
            @csrf
