...
/**
 * Sekarang jadi WRAPPER ke tabel database "produks" (Model Produk),
 * bukan lagi array statis. Bentuk data yang dikembalikan tetap sama
 * seperti sebelumnya (array asosiatif per slug) supaya semua kode lain
 * yang sudah memanggil ProdukData::cari()/::semua()/::terkait() tidak
 * perlu diubah.
 */
class ProdukData
{
    ...
    {
        return Produk::query()->orderBy('id');
    }

    public static function semua(): array
    {
        $produk = self::query()->get()
            ->keyBy('slug')
            ->map(fn (Produk $p) => $p->toArray())
            ->all();
    ...
    {
        $produk = self::query()->where('slug', $slug)->first();
        if ($produk) {
            return $produk->toArray();
        }

        return self::fallbackData()[$slug] ?? null;
    }

    /**
     * Ambil beberapa produk lain untuk widget "Mungkin Kamu Suka",
     * mengecualikan produk yang sedang dilihat.
     */
    public static function terkait(string $slugSaatIni, int $jumlah = 4): array
    {
        $produk = self::query()->where('slug', '!=', $slugSaatIni)
            ->take($jumlah)
            ->get()
            ->keyBy('slug')
            ->map(fn (Produk $p) => $p->toArray())
        ...
        return collect(self::fallbackData())
            ->except($slugSaatIni)
            ->take($jumlah)
            ->all();
    }
